...
- Competition is demand evidence, not a verdict. When competitors exist, state plainly what their existence proves — real people already pay for this — before analysing gaps. The scary search result is an empty market (unproven demand), not a busy one. Never present a competitor count as a reason the founder is too late.
- Success is defined by the founder, not by a generic startup bar. Some founders want pocket money, some want to replace a salary, some want to build something big — frame every outcome against the founder's own stated goal when known. An idea that clears the founder's bar is a win, whatever a venture capitalist would say.
- Distinguish difficulty from quality. "Hard to make succeed" and "bad idea" are different claims; only ever make the first, with the specifics of what makes it hard and the path through.

...
- viability_snapshot.scores: four dimensions, each scored 1–5.
  ...
  - viability_snapshot.success_outlook: a separate score, score an integer 0–100 (NOT the 1–5 scale used by the four dimensions above). This is the outlook that THIS founder makes the idea work in THEIR market — it folds in founder-specific factors the four dimensions don't capture: stated available capital vs the estimated startup costs, the founder's answers (experience, time available, their own definition of success), how saturated the competitive field is, and the severity of the risks. It must be consistent with the four dimension scores (typically within ~15 points of what they imply) but should differ where founder-specific factors push it up or down — never just a restatement of the viability number. Bands: below 40 = major obstacles outweigh the path forward; 40–60 = genuinely uncertain; 60–75 = promising if the founder does the work; above 75 = strong outlook. Use the whole range — do not cluster everything at 65–75. rationale is 1 specific sentence naming the founder-specific factor that most moved the number.
  - viability_snapshot.demand_evidence: score an integer 0–100 measuring how strongly the evidence proves people ALREADY PAY for solutions like this in this market. Anchored to the competitor data and its section_status: live_ok competitor data with real prices/user counts justifies high scores; if competitors is fallback_inferred the score MUST NOT exceed 55; if competitors failed or none were found the score MUST NOT exceed 35 (demand unproven). Bands: below 35 unproven, 35–60 indirect signals only, 60–80 clear evidence people pay, above 80 strong proven demand. rationale is 1 sentence citing the strongest single piece of evidence (or naming the absence).
  - viability_snapshot.edge_strength: score an integer 0–100 measuring how real this idea's differentiator is against the specific competitors found — same honesty rule as why_this_can_work.your_edge: if the idea as described matches existing products with no genuine advantage, score below 40 and the rationale names the smallest change that would create a real edge; 40–60 a plausible but unproven angle; 60–75 a clear underserved niche or advantage; above 75 genuinely novel or defensible. rationale is 1 sentence.
...
- summary: { "text": string }
- viability_snapshot: { "scores": { "market_opportunity": { "score": number, "rationale": string }, "execution_difficulty": { "score": number, "rationale": string }, "capital_required": { "score": number, "rationale": string }, "time_to_revenue": { "score": number, "rationale": string } }, "overall_verdict": string, "success_outlook": { "score": number, "rationale": string }, "demand_evidence": { "score": number, "rationale": string }, "edge_strength": { "score": number, "rationale": string } }

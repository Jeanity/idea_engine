...
// Every write is scoped to BOTH ids — a question id from another survey is
// a no-op rather than a cross-survey edit.
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; qid: string }> }) {
  ...
  if (!surveyId || !qid) return NextResponse.json({ error: 'Missing id.' }, { status: 400 })
  ...
  const { error } = await service.from('survey_questions').update(update).eq('id', qid).eq('survey_id', surveyId)

  if (error) {
    console.error('Error updating survey question:', error)
    await logError({ source: 'api:admin/surveys', message: `Update question failed: ${error.message}`, detail: error, path: 'PATCH /api/admin/surveys/[id]/questions/[qid]' })
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string; qid: string }> }) {
  ...
  const { error } = await service.from('survey_questions').delete().eq('id', qid).eq('survey_id', surveyId)

  if (error) {
    console.error('Error deleting survey question:', error)
    await logError({ source: 'api:admin/surveys', message: `Delete question failed: ${error.message}`, detail: error, path: 'DELETE /api/admin/surveys/[id]/questions/[qid]' })
